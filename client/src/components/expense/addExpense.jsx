...
export default function AddExpense() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      const payload = {
        ...
        expenseAmount: parseFloat(values.expenseAmount), // Ensure it's a number
        expenseCategory: values.expenseCategory,
        expenseDate: values.expenseDate, // Ensure the backend supports the date format
        expenseMembers: values.expenseMembers, // Pass as an array
        expenseOwner: values.expenseOwner,
  ...
  const handleFileUpload = async (event) => {
    ...
    setExpenseImage(file); // Store the file in state
    formik.setFieldValue("expenseFile", file);
    ...
    try {
      ...
      formik.setFieldValue("expenseName", parsedData.Name || "");
      formik.setFieldValue("expenseDescription", parsedData.Description || "");
      formik.setFieldValue("expenseAmount", parsedData["Total Amount"] || "");
      formik.setFieldValue("expenseCategory", parsedData.Category || "Others");
      ...
      setAlertMessage("Failed to extract data from the uploaded image.");
      setAlert(true);
    }
  };
              ...
              <Grid container spacing={3} sx={{ maxWidth: 800 }}>
                <Grid item xs={12}>
                  <TextField
                    ...
                    type="file"
                    name="expenseImage"
                    id="outlined-expense-image"
                    label="Upload bill Image to auto fill data(optional)"
                    InputLabelProps={{ shrink: true }}
                    variant="outlined"
                    inputProps={{ accept: "image/*" }}
                    onChange={(event) => handleFileUpload(event)}
                    error={Boolean(touched.expenseImage && errors.expenseImage)}
                    helperText={touched.expenseImage && errors.expenseImage}
                  />
                </Grid>
